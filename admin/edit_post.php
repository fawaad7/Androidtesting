<?php session_start();
	...
	if(isset($_POST['update_post'])){
		...
		if($_FILES['image']['name'] != ''){
			...
			if($image_size < 1000000){
				if($image_ext == 'jpg' || $image_ext == 'png' || $image_ext == 'gif'){
					if(move_uploaded_file($image_tmp, $image_path)){
						$up_sql = "UPDATE `posts` SET `title` = '$title',`image` = '$image_db_path', `description` = '$_POST[description]', `category` = '$_POST[category]', `status` = '$_POST[status]' WHERE `id` = '$_POST[post_id]'";
						if(mysqli_query($conn, $up_sql)){
							$result = '<div class="alert alert-danger">You\'ve Edited the post</div>';
						}else {
			...
			$up_sql = "UPDATE posts SET title='$title', description='$_POST[description]', category='$_POST[category]', status='$_POST[status]' WHERE id = '$_POST[post_id]'";
			if(mysqli_query($conn, $up_sql)){
						...
						<select id="status" name="status" class="form-control">
							<?php 
								if($rows['status'] == 'draft'){
									...
									echo '<option value="draft">Draft</option><option value="publish" selected>Publish</option>';
								}
							?>
